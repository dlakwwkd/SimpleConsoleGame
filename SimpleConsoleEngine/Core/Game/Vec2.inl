...
inline const Vec2   Vec2::operator+ (const Vec2 &v) const noexcept  { return Vec2(m_X + v.m_X, m_Y + v.m_Y); }
inline const Vec2   Vec2::operator- (const Vec2 &v) const noexcept  { return Vec2(m_X - v.m_X, m_Y - v.m_Y); }
inline const Vec2   Vec2::operator* (float s)       const noexcept  { return Vec2(m_X * s, m_Y * s); }
inline const Vec2   Vec2::operator/ (float s)       const noexcept  { return Vec2(m_X / s, m_Y / s); }
inline const Vec2   Vec2::operator- ()              const noexcept  { return Vec2(-m_X, -m_Y); }

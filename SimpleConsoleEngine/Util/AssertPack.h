...
namespace Assert
{
    template<typename T, typename S>        inline constexpr void   OverflowCheck(const S& source) noexcept;
}

// Assert를 통해 디버그 빌드에선 절대 안전성을 보장하는 함수들
namespace Safe
{
    template<typename T, typename S>        inline constexpr T      IntegralCast(const S& source) noexcept;
    template<typename T, typename... Args>  inline T*               New(Args&&... args) noexcept;
